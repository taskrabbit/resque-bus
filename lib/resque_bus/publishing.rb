module ResqueBus
  module Publishing
    ...
    def push(queue, item)
      watch_queue(queue)
      ResqueBus.redis.rpush "queue:#{queue}", Resque.encode(item)
    end
    
    # Used internally to keep track of which queues we've created.
    # Don't call this directly.
    def watch_queue(queue)
      ResqueBus.redis.sadd(:queues, queue.to_s)
    end
    ...
    def delayed_push(timestamp, item)
      ...
      ResqueBus.redis.rpush("delayed:#{timestamp.to_i}", Resque.encode(item))

      # Now, add this timestamp to the zsets.  The score and the value are
      # the same since we'll be querying by timestamp, and we don't have
      # anything else to store.
      ResqueBus.redis.zadd :delayed_queue_schedule, timestamp.to_i, timestamp.to_i
    end
